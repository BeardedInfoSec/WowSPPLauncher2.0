...
namespace SppLauncher.Windows.Launcher
{
    public partial class Launcher : Form
    {
        ...
        public void ExpansionSelection(ServerConfig.ExpansionType Expansion)
        {
        ...
        public void CloseProcess(bool restart)
        {
            try
            {
                ...
                foreach (var proc in Process.GetProcessesByName("realmd"))
                {
        ...
        private void tmrUsage_Tick(object sender, EventArgs e)
        {
            ...
            catch (Exception)
            {
                ...
                realmdProcesses = Process.GetProcessesByName("realmd");
        ...
        private void SqlStartCheck_Tick(object sender, EventArgs e)
        {
            try
            {
                if (_sql.Contains("Version: '5.7.26'  socket: ''  port: 3310  MySQL Community Server (GPL)"))
                {
                    MysqlOn = true;
                    SqlStartCheck.Stop();
                    var end1        = DateTime.Now;
